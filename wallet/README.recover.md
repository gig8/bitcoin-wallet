Recovering MotaCoins
===================

## PROLOGUE

This document describes how you can use a backup file on a standard PC to recover your MotaCoins.
Normally, this shouldn't be needed. It is much preferred to just use **Options > Safety > Restore wallet** from within the MotaCoin Wallet app if you can. This guide is only meant for rare cases:
...
If you followed the app's guidance your backup files will be located on a share of the storage access framework, very likely your Google Drive. Watch out for filenames starting with `motacoin-wallet-backup`.
...
You now have your backup file on your PC. Wallet backups are encrypted. Let's decrypt it using:
    ...
    openssl enc -d -aes-256-cbc -md md5 -a -in motacoin-wallet-backup-testnet-2014-11-01 > motacoin-wallet-decrypted-backup
...
Historically there is two backup formats. Let's look at the first printable characters in the file:
    ...
    cat motacoin-wallet-decrypted-backup | tr -cd "[:print:]" | awk '{print $1}'

If it prints "org.motacoin.production", you got the right password and the backup file uses the
motacoinj protobuf format. This backup format was introduced in v3.47 (May 2014). Skip to
...
We need wallet-tool from motacoinj. First, in a working directory, let's get motacoinj:

    git clone -b release-0.14 https://github.com/motacoinj/motacoinj.git
...
Now use wallet-tool to sync the wallet from your backup:
    ...
    ./wallet-tool reset --wallet=/tmp/motacoin-wallet-decrypted-backup
    ./wallet-tool sync --wallet=/tmp/motacoin-wallet-decrypted-backup --debuglog
...
shell prompt if its finished syncing. Have a look at the wallet:
    ...
    ./wallet-tool dump --wallet=/tmp/motacoin-wallet-decrypted-backup
...
your entire wallet to the desired destination wallet:
    ...
    ./wallet-tool send --wallet=/tmp/motacoin-wallet-decrypted-backup --output=<receiving address of destination wallet>:ALL
...
You can also get a list of the private keys. If your wallet has a spending PIN set you need to decrypt it first, otherwise the private keys won't appear. Note that when you decrypt the wallet *the private keys can be accessed (and your MotaCoins stolen) by anyone with access to the system*, including malware or other users. Unless you fully trust the security of the computer consider running it on an offline system with no network connectivity.

    ./wallet-tool decrypt --wallet=/tmp/motacoin-wallet-decrypted-backup --password=<PIN>

Then to get the private keys use:

    ./wallet-tool dump --wallet=/tmp/motacoin-wallet-decrypted-backup --dump-privkeys
...
Have a deeper look at the backup file:
    ...
    cat motacoin-wallet-decrypted-backup
...
or [MotaCoin Core](https://motacoincore.org/).
